...
@testitem "chebgrid1_integration_matrix" begin
    n = 10
    grid = ChebyshevGrid(n, 0.0, 1.0; kind=1)
    Q = chebgrid1_integration_matrix(n, 0.0, 1.0)
    @test size(Q) == (n, n)

    # Test scaling with interval change
    lower_bound, upper_bound = -2.0, 3.0
    Q_scaled = chebgrid1_integration_matrix(Float64, n, lower_bound, upper_bound)
    scale = (upper_bound - lower_bound) / 2
    Q_default = chebgrid1_integration_matrix(n)
