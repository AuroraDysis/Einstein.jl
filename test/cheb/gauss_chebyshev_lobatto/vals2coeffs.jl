...
@testitem "GaussChebyshevLobattoGrid - vals2coeffs" begin
    # Set tolerance
    tol = typetol(Float64)

    function vals2coeffs_via_matrix(v::AbstractVector{T}) where {T<:Number}
        n = length(v)
        A = GaussChebyshevLobattoGrid.vals2coeffs_matrix(T, n)
        return A * v
    end

    @testset "Single value" begin
        v = sqrt(2)
        c1 = GaussChebyshevLobattoGrid.vals2coeffs([v])
    ...
    @testset "Array input" begin
        ...
        c1 = GaussChebyshevLobattoGrid.vals2coeffs(v)
        c2 = GaussChebyshevLobattoGrid.vals2coeffs(reverse(v))
    ...
    @testset "Symmetry preservation" begin
        ...
        c_even = GaussChebyshevLobattoGrid.vals2coeffs(v_even)
        c_odd = GaussChebyshevLobattoGrid.vals2coeffs(v_odd)
    ...
    @testset "Operator style" begin
        ...
        # Test operator call
        coeffs1 = op(vals)
        coeffs2 = GaussChebyshevLobattoGrid.vals2coeffs(vals)
