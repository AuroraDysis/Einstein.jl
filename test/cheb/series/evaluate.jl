@testitem "cheb_series_evaluate" begin
    using LinearAlgebra
    ...
    @testset "Single coefficient tests" begin
        # Scalar evaluation
        c = [sqrt(2)]
        v = cheb_series_evaluate(c, 0.0)
        @test v ≈ sqrt(2)

        # Vector evaluation
        x = [-0.5, 1.0]
        v = cheb_series_evaluate(c, x)
        @test all(v .≈ sqrt(2))
    end
    ...
    @testset "Vector coefficient tests" begin
        ...
        c2 = reverse(c)
        v = map(xi -> [cheb_series_evaluate(c, xi), cheb_series_evaluate(c2, xi)], x)
        v_true = [
            3.0 0.0
            3.1728 3.6480
            15.0 15.0
    ...
    @testset "Edge cases" begin
        ...
        @test_throws ArgumentError cheb_series_evaluate(Float64[], 0.0)

        # Single coefficient
        @test cheb_series_evaluate([1.0], 0.5) ≈ 1.0
    end
end
