@testitem "fdm_centralwts" begin
    using Einstein.FiniteDifferenceSuite, Test

    # Test 1st derivative, 2nd order accuracy
    stencil = [-1//2, 0//1, 1//2]
    @test fdm_centralwts(1, 2) == stencil
    for type in [Float64, BigFloat]
        @test fdm_centralwts(type, 1, 2) ≈ type.(stencil)
    end

    # Test 2nd derivative, 2nd order accuracy 
    stencil = [1//1, -2//1, 1//1]
    @test fdm_centralwts(2, 2) ≈ stencil
    for type in [Float64, BigFloat]
        @test fdm_centralwts(type, 2, 2) ≈ type.(stencil)
    end

    # Test 1st derivative, 4th order accuracy
    stencil = [1//12, -2//3, 0//1, 2//3, -1//12]
    @test fdm_centralwts(1, 4) == stencil
    for type in [Float64, BigFloat]
        @test fdm_centralwts(type, 1, 4) ≈ type.(stencil)
    end
    ...
    @test fdm_centralwts(6, 6) ≈ stencil
    for type in [Float64, BigFloat]
        @test fdm_centralwts(type, 6, 6) ≈ type.(stencil)
    end
end

@testitem "fdm_hermitewts" begin
    using Einstein.FiniteDifferenceSuite, Test

    # Test 2nd derivative, 4th order accuracy
    D2, E2 = fdm_hermitewts(2, 4)
    @test D2 == [2//1, -4//1, 2//1]
    @test E2 == [1//2, 0//1, -1//2]

    # Test 2th derivative, 8nd order accuracy
    D4, E4 = fdm_hermitewts(2, 8)
