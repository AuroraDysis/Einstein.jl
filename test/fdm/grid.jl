@testitem "fdm_grid" begin
    ...
    @testset "Basic functionality" begin
        # Integer-spaced grid
        grid = fdm_uniform_grid(0.0, 1.0, 0.1)
        @test length(grid) == 11
        @test grid[1] ≈ 0.0
        @test grid[end] ≈ 1.0
    ...
    @testset "Error cases" begin
        # Invalid interval
        @test_throws ArgumentError fdm_uniform_grid(1.0, 0.0, 0.1)

        # Invalid spacing
        @test_throws ArgumentError fdm_uniform_grid(0.0, 1.0, 0.0)
        @test_throws ArgumentError fdm_uniform_grid(0.0, 1.0, -0.1)

        # Test endpoint mismatch case
        # This should throw an error when dx doesn't divide interval evenly
        @test_throws ArgumentError fdm_uniform_grid(0.0, 1.0, 0.3)
    end
end