...
@testitem "fdm_fornbergwts" begin
    ...
    @testset "Standard First Derivatives" begin
        ...
        weights_forward = fdm_fornbergwts(1, 0.0, [0.0, 1.0])
        @test weights_forward ≈ [-1.0, 1.0]

        # Test central difference (first derivative)
        weights_central = fdm_fornbergwts(1, 0.0, [-1.0, 0.0, 1.0])
        @test weights_central ≈ [-0.5, 0.0, 0.5]

        # Test accuracy with non-uniform grid
        weights_nonuniform = fdm_fornbergwts(1, 0.0, [-2.0, -0.5, 1.0])
    ...
    @testset "Standard Second Derivatives" begin
        ...
        weights_central = fdm_fornbergwts(2, 0.0, [-1.0, 0.0, 1.0])
        @test weights_central ≈ [1.0, -2.0, 1.0]

        # Test with wider stencil
        weights_wide = fdm_fornbergwts(2, 0.0, [-2.0, -1.0, 0.0, 1.0, 2.0])
