...
@testitem "fdm_weights" begin
    ...
    @testset "Standard Second Derivatives" begin
        ...
        weights_central = fdm_weights(2, 0.0, [-1.0, 0.0, 1.0])
        @test weights_central ≈ [1.0, -2.0, 1.0]

        # Test with wider stencil
        weights_wide = fdm_weights(2, 0.0, [-2.0, -1.0, 0.0, 1.0, 2.0])
    ...
    @testset "Hermite Finite Difference" begin
        ...
        x = [-1.0, 0.0, 1.0]
        D, E = fdm_weights(1, 0.0, x; hermite=true)
        ...
        D2, E2 = fdm_weights(2, 0.0, x; hermite=true)
        @test sum(D2) ≈ 0.0 atol = 1e-14
    end

    @testset "Error Handling" begin
        # Test insufficient points for order
        @test_throws ArgumentError fdm_weights(2, 0.0, [0.0, 1.0])

        # Test insufficient points for Hermite interpolation
        @test_throws ArgumentError fdm_weights(3, 0.0, [0.0, 1.0], hermite=true)
    end
end
