@testitem "FDM Convolution Operations" begin
    ...
    @testset "Interior Convolution" begin
        ...
        jacobian = [0.5, 0.6, 0.7, 0.8, 0.9]
        fdm_convolve_interior!(out_1d, in_1d, weights, factor, jacobian)
        @test out_1d[2:4] ≈ [0.6, 0.7, 0.8]

        # Test 2D interior convolution with vector factor
        fdm_convolve_interior!(out_2d, in_2d, weights, factor, jacobian)
        ...
        out_1d_add = ones(size(in_1d))
        fdm_convolve_interior!(out_1d_add, in_1d, weights, factor, jacobian, ConvolveAdd())
        @test out_1d_add[2:4] ≈ [1.6, 1.7, 1.8]

        # Test Add mode for 2D interior convolution with vector factor
        out_2d_add = ones(size(in_2d))
        fdm_convolve_interior!(out_2d_add, in_2d, weights, factor, jacobian, ConvolveAdd())
    ...
    @testset "Boundary Convolution" begin
        ...
        # Test with vector jacobian
        jacobian = [0.5, 0.6, 0.7, 0.8, 0.9]
        fdm_convolve_boundary!(out_1d, in_1d, left_weights, right_weights, factor, jacobian)
        ...
        # Test with vector jacobian
        fdm_convolve_boundary!(out_2d, in_2d, left_weights, right_weights, factor, jacobian)
        ...
        # Test Add mode with vector jacobian
        out_1d_add = ones(size(in_1d))
        fdm_convolve_boundary!(out_1d_add, in_1d, left_weights, right_weights, factor, jacobian, ConvolveAdd())
        ...
        # Test Add mode with vector jacobian for 2D
        out_2d_add = ones(size(in_2d))
        fdm_convolve_boundary!(out_2d_add, in_2d, left_weights, right_weights, factor, jacobian, ConvolveAdd())
