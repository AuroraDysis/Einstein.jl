"""
    cheb1_barycentric_weights([TF=Float64], n::Integer) where {TF<:AbstractFloat}
...
"""
function cheb1_barycentric_weights(::Type{TF}, n::Integer) where {TF<:AbstractFloat}
    if n == 0
        return TF[]
    elseif n == 1
        return TF[one(TF)]
...
function cheb1_barycentric_weights(n::Integer)
