...
"""
    fornberg_calculate_weights([T=Float64], order::Integer, x0::Real, x::AbstractVector; 
                             dfdx::Bool=false)
...
export fornberg_calculate_weights
...
@testset "Fornberg Finite Difference Weights" begin
    @testset "Standard First Derivatives" begin
        ...
        weights_forward = fornberg_calculate_weights(1, 0.0, [0.0, 1.0])
        @test weights_forward ≈ [-1.0, 1.0]

        # Test central difference (first derivative)
        weights_central = fornberg_calculate_weights(1, 0.0, [-1.0, 0.0, 1.0])
        @test weights_central ≈ [-0.5, 0.0, 0.5]

        # Test accuracy with non-uniform grid
        weights_nonuniform = fornberg_calculate_weights(1, 0.0, [-2.0, -0.5, 1.0])
    ...
    @testset "Standard Second Derivatives" begin
        ...
        weights_central = fornberg_calculate_weights(2, 0.0, [-1.0, 0.0, 1.0])
        @test weights_central ≈ [1.0, -2.0, 1.0]

        # Test with wider stencil
        weights_wide = fornberg_calculate_weights(2, 0.0, [-2.0, -1.0, 0.0, 1.0, 2.0])
    ...
    @testset "Hermite Finite Difference" begin
        ...
        x = [-1.0, 0.0, 1.0]
        D, E = fornberg_calculate_weights(1, 0.0, x; dfdx=true)
        ...
        D2, E2 = fornberg_calculate_weights(2, 0.0, x; dfdx=true)
        @test sum(D2) ≈ 0.0 atol = 1e-14
    end

    @testset "Error Handling" begin
        # Test insufficient points for order
        @test_throws ArgumentError fornberg_calculate_weights(2, 0.0, [0.0, 1.0])

        # Test insufficient points for Hermite interpolation
        @test_throws ArgumentError fornberg_calculate_weights(3, 0.0, [0.0, 1.0], dfdx=true)
    end
end
